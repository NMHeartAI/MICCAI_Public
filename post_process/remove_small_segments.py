...
# segment_pixel_counts -- a list to store the pixel numbers of all divided segments
# segment_images -- a list of segment images
# threshold -- remove segments smaller than the threshold
    
    out_seg = copy.deepcopy(segment_images)
    out_counts = copy.deepcopy(segment_pixel_counts)
    for i, pixel_count in enumerate (segment_pixel_counts):
        if pixel_count < threshold:
            out_counts.remove(pixel_count)
            to_remove = segment_images[i]
...
def binary_remove_small_segments(binary_image, threshold):
    # Remove segments in a binary image smaller than the threshold
